...
fuzz_target!(|data: &[u8]| {
    ...
    // Fix total_minted to match total_supply
    state.total_minted = state.total_supply;
    state.total_burned = 0;

    // Fix total_base_sum safely with overflow checks
    if let Some(field_contrib) = state.global_field.checked_mul(state.p as i128) {
        state.total_base_sum = (state.total_supply as i128).saturating_sub(field_contrib);
    } else {
        // Overflow: reset to safe state
        state.global_field = 0;
        state.total_base_sum = state.total_supply as i128;
    }
    ...
    for _ in 0..steps {
        ...
        if state.check_invariant().is_err() {
            panic!("invariant violated: {:?}", state);
        }
    }
});
